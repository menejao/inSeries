...
export default async function WatchlistPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs items={tabs} active="/me/watchlist" />
      <div className="grid gap-4 lg:grid-cols-2">
        {series.length ? series.map((item) => <SeriesCard key={item.id} series={item} />) : <EmptyState title="Watchlist vazia" copy="Use Quero assistir para montar lista." />}
      </div>
    </div>
  );
}
