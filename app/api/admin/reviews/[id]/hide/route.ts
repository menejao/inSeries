...
import { hideReview } from "@/lib/admin/moderation";
...
async function hideHandler(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await hideReview(admin.id, id);
...
export const POST = withApiObservability("admin.reviews.hide", hideHandler);
