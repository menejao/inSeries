...
import { restoreList } from "@/lib/admin/moderation";
...
async function restoreHandler(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await restoreList(admin.id, id);
...
export const POST = withApiObservability("admin.lists.restore", restoreHandler);
