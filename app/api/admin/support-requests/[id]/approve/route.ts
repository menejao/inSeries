...
import { approveSupportRequest } from "@/lib/supporters/admin";
...
async function approveHandler(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await approveSupportRequest(admin.id, id, payload.data.notes);
  if (!result.ok) return NextResponse.json({ error: result.error }, { status: errorStatus(result.error) });

  return NextResponse.json({ ok: true });
}

export const POST = withApiObservability("admin.support-requests.approve", approveHandler);
