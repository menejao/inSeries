...
import { syncOnTheAirSeries } from "@/lib/catalog/sync";
...
async function main() {
  ...
  const summary = await syncOnTheAirSeries({ pages });
  ...
  printSyncSummary("Sync de series em exibicao (On The Air) concluido.", summary);
...
main()
  .catch((error) => {
    console.error("Falha inesperada no sync de series em exibicao.");
    console.error(error);
    process.exitCode = 1;
  })
  .finally(async () => {
