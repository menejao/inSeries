...
import { syncAiringTodaySeries } from "@/lib/catalog/sync";
...
async function main() {
  ...
  const summary = await syncAiringTodaySeries({ pages });
  ...
  printSyncSummary("Sync de series no ar hoje (Airing Today) concluido.", summary);
...
main()
  .catch((error) => {
    console.error("Falha inesperada no sync de series no ar hoje.");
    console.error(error);
    process.exitCode = 1;
  })
  .finally(async () => {
