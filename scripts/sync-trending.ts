...
import { syncTrendingSeries } from "@/lib/catalog/sync";
...
async function main() {
  ...
  const window = process.argv[3] === "day" ? "day" : "week";

  const summary = await syncTrendingSeries({ pages, window });
  ...
  printSyncSummary(`Sync de series em alta (Trending, ${window}) concluido.`, summary);
...
main()
  .catch((error) => {
    console.error("Falha inesperada no sync de series em alta.");
    console.error(error);
    process.exitCode = 1;
  })
  .finally(async () => {
